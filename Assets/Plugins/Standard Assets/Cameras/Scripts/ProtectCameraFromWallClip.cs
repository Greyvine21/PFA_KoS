...
namespace UnityStandardAssets.Cameras
{
    public class ProtectCameraFromWallClip : MonoBehaviour
    {
        ...
        //public LayerMask dontClipLayer;           // don't clip against objects with this tag (useful for not clipping against the targeted object)
        ...
        private void LateUpdate()
        {
            ...
            for (int i = 0; i < cols.Length; i++)
            {
                ...
                {
                    //print("collide  " + cols[i].gameObject.tag);
            ...
            for (int i = 0; i < m_Hits.Length; i++)
            {
                    ...
                    if (m_Hits[i].distance < nearest && (!m_Hits[i].collider.isTrigger) && !m_Hits[i].collider.CompareTag(dontClipTag))
                    {
                        //print("hit  " + m_Hits[i].collider.tag);
