...
// Water plane at y = 0
	...
	//private Vector3[] wposTemp;
	
	void Start ()
	{
		//GetComponent<Rigidbody>().centerOfMass = new Vector3 (0.0f, -1f, 0.0f);
		GetComponent<Rigidbody>().centerOfMass = m_CenterMass.position;
	...
	void OnDrawGizmos()
	{
		...
		/*for (int i = 0; i < wposTemp.Length; i++)
		{
			Gizmos.DrawWireSphere(wposTemp[i], 1);
			print(wposTemp[i]);
		}*/
	}
