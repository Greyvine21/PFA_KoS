...
public class WaterController : MonoBehaviour 
{
    ...
    public TypeOfWater m_WaveType;

    public Ocean m_ocean;
    ...
    public float GetWaveYPos(Vector3 position, float timeSinceStart)
    {
        ...
        if (isMoving)
        {
            return WaveTypes.SinXWave(m_WaveType, position, speed, scale, waveDistance, noiseStrength, noiseWalk, timeSinceStart);
        }
        else
        {
            return 0f;
		...
		/*/
		return m_ocean.GetWaterHeightAtLocation (position.x, position.z);
        //*/
    }
